...
export class RolesService extends BaseService<Role, SaveRoleDto> {
  ...
  async save(createDto: SaveRoleDto) {
    ...
    if (
      isAdminRole &&
      !this.hasPermission(operatorPermissions, "system/roles/manageAdminRole")
    ) {
  ...
  async getUserMenus(
    user: {
      name?: string;
      roles?: Array<{ permissionKey?: string; isActive?: string }>;
    },
    isTree = false,
  ): Promise<Menu[]> {
  ...
  async dataValidate(data: { id; updateUser; permissions?: string[] }) {
    ...
    if (!row) return true;
    if (
      row.permissionKey === config.adminKey &&
      !this.hasPermission(
        data.permissions || [],
  ...
  private async ensureManageRolePermission(
    ...
    if (
      ...
      !this.hasPermission(permissions || [], "system/roles/manageAdminRole")
    ) {
      throw new HttpException("接口无权限", 403);
    }
  }
