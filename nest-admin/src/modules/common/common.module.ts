...
export class StaticFileMiddleware implements NestMiddleware {
  use(req: Request, res: Response, next: NextFunction) {
    // 检查请求是否匹配静态资源路径 - 支持 /static/ /upload/ 和 /api/ 两种路径
    const isStaticPath = req.url.startsWith('/static/') || req.url.startsWith('/upload/') || req.url.startsWith('/api/static/') || req.url.startsWith('/api/upload/')
    if (isStaticPath) {
      // 获取文件路径
      let filePath = req.url;
      // 移除路径前缀
      if (filePath.startsWith('/api/upload/')) {
        filePath = filePath.replace('/api/upload/', '');
      } else if (filePath.startsWith('/api/static/')) {
