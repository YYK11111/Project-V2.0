...
export class MenusService extends BaseService<Menu, CreateMenuDto> {
  ...
  async list(query: QueryListDto = {}, isTree = true) {
    ...
    if (type && data.length > 0) {
      const parentIds = [
        ...new Set(
          data.map((item) => item.parentId).filter((id) => id && id !== "0"),
        ),
      ];
    ...
    return isTree
      ? this.sortMenuTree(arrayToTree(data))
      : this.sortMenuList(data);
  }
  ...
  async getDiagnostics(): Promise<MenuDiagnosticSummary> {
    ...
    menus.forEach((menu) => {
      if (menu.type === MenuType.menu && !String(menu.component || "").trim()) {
        missingComponentMenus.push(
          this.toDiagnosticIssue(menu, "菜单缺少组件配置"),
        );
      }

      if (
        menu.isHidden === "1" &&
        this.constantHiddenPaths.has(this.normalizePath(menu.path))
      ) {
        redundantHiddenRoutes.push(
          this.toDiagnosticIssue(menu, "与前端常量隐藏路由重复"),
        );
      }
    ...
    const duplicateSiblingPaths = [...duplicateSiblingPathMap.entries()]
      ...
      .map(([routeKey, sameMenus]) => {
        ...
        return {
          ...
          suggestion:
            "合并重复菜单，只保留一个有效路由，其余菜单迁移角色绑定后删除",
        };
      });
  ...
  private async validateMenuConfig(
    data: SaveDto<Menu> & {
      id?: string;
      parentId?: string;
      path?: string;
    ...
    const duplicateSibling = siblingMenus.find((item) => {
      ...
      return (
        this.normalizePath(item.path) === normalizedPath &&
        item.type === menuType &&
        item.isDelete == null
      );
    });
