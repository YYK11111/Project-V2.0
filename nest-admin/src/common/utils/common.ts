...
export function arrayToTree(data, id = 'id', parentId = 'parentId', children = 'children', rootId = '0') {
  ...
  console.log('[arrayToTree] 原始数据条数:', data.length)
  console.log('[arrayToTree] 示例数据:', data.slice(0, 2).map(item => ({ id: item[id], parentId: item[parentId], path: item.path })))
  
  data.forEach((item) => {
    delete item[children]
    // 统一将 id 转为字符串，避免 bigint 类型导致的类型不匹配问题
    item[id] = String(item[id])
    item[parentId] = item[parentId] !== null && item[parentId] !== undefined ? String(item[parentId]) : null
  })
  
  console.log('[arrayToTree] 转换后的示例数据:', data.slice(0, 2).map(item => ({ id: item[id], parentId: item[parentId], path: item.path })))
  ...
  console.log('[arrayToTree] Map keys:', Object.keys(map).slice(0, 5))
  ...
  data.forEach((item) => {
    ...
    } else if (!rootId || item[parentId] === rootId || item[parentId] === null || item[parentId] === '0') {
      result.push(item)
    }
  })
  
  console.log('[arrayToTree] 根节点数量:', result.length)
  console.log('[arrayToTree] 根节点示例:', result.slice(0, 2).map(item => ({ id: item[id], path: item.path, childrenCount: item[children]?.length || 0 })))
