...
const iv = Uint8Array.from([
  40, 157, 36, 126, 11, 144, 3, 41, 39, 8, 79, 210, 230, 222, 167, 91,
]);
...
const rawKey = Uint8Array.from([
  194, 181, 53, 234, 250, 85, 168, 205, 114, 199, 141, 228, 115, 99, 93, 17,
  234, 15, 185, 94, 247, 13, 104, 251, 146, 183, 87, 41, 29, 153, 44, 32,
]);

async function generateAesKey(mode = "AES-GCM", length = 256) {
  let key = await subtle.generateKey({ name: mode, length }, true, [
    "encrypt",
    "decrypt",
  ]);
  // 导出
...
async function importKey(rawKey, mode = "AES-GCM") {
  return await subtle.importKey("raw", rawKey, mode, true, [
    "encrypt",
    "decrypt",
  ]);
}
...
export async function decrypt(str, mode = "AES-GCM") {
  let res = await subtle.decrypt(
    { name: mode, iv: iv },
    await importKey(rawKey),
    str2ab(globalThis.atob(str)),
  );
