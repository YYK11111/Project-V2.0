...
export class BaseService<T, K> {
  ...
  async del(
    ...
    return {
      ...result,
      successCount: normalizedIds.length,
      failedCount: 0,
      successIds: normalizedIds,
      failed: [],
    } as any;
  }

  async getOne(
    query: (FindOptionsWhere<T> & FindOneOptions) | {},
    isError = true,
  ): Promise<any | null> {
  ...
  async listBy(
    ...
    pageNum &&
      pageSize &&
      ((queryOrm.skip = --pageNum * pageSize), (queryOrm.take = pageSize));
  ...
  sqlLike(value) {
    return value == undefined
      ? undefined
      : Like(`%${value.replace(/%/g, "\\%").replace(/_/g, "\\_")}%`);
  ...
  betweenDateArr(beginEndTime: [string, string]) {
    return (
      ...
      beginEndTime?.[1] &&
      Array.from(
        { length: dayjs(beginEndTime[1]).diff(beginEndTime[0], "day") + 1 },
        (_, i) => dayjs(beginEndTime[0]).add(i, "day").format("YYYY-MM-DD"),
      )
    );
  }
