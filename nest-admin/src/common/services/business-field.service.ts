...
export class BusinessFieldService {
  ...
  async getFieldMappingsForBusinessType(
    businessType: string,
  ): Promise<BusinessFieldOption[]> {
    return this.getFieldOptions(businessType);
  }

  async getAllFieldOptionsForBusinessType(
    businessType: string,
  ): Promise<BusinessFieldOption[]> {
  ...
  async generateAndSaveFieldMappings(businessType: string): Promise<number> {
    const fields =
      this.dataLoaderService.getFieldDefinitions(businessType) || [];
  ...
  private async getFieldOptions(
    ...
    return fields
      .filter((field) =>
        options.includeAll ? true : this.isAssigneeField(field),
      )
      .map((field, index) => ({
      ...
      .sort(
        (a, b) =>
          a.order - b.order ||
          a.fieldLabel.localeCompare(b.fieldLabel, "zh-CN"),
      );
  }
  ...
  private buildFieldDescription(field: FieldSource, includeAll = false) {
    ...
    if (
      includeAll &&
      ["number", "date", "boolean", "enum"].includes(field.type)
    ) {
  ...
  private getFieldGroup(field: FieldSource, includeAll = false) {
    if (includeAll) {
      ...
      if (this.normalizeFieldType(field) === "departmentArray")
        return "部门集合字段";
  ...
  private normalizeFieldType(field: FieldSource) {
    if (field.type === "array") {
      if (field.label?.includes("部门") || /dept|department/i.test(field.name))
    ...
    if (field.type === "string") {
      if (field.label?.includes("部门") || /dept|department/i.test(field.name))
        return "department";
      if (
        field.label?.includes("负责人") ||
        field.label?.includes("申请人") ||
        field.label?.includes("审批人") ||
        field.label?.includes("提交人") ||
        field.label?.includes("处理人") ||
        field.label?.includes("经办人")
      ) {
        return "user";
      }
    }
