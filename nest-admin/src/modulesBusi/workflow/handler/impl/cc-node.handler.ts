...
export class CcNodeHandler implements INodeHandler {
  ...
  async execute(context: NodeExecutionContext): Promise<NodeResult> {
    ...
    try {
      const messageSummary = this.getMessageSummary(
        context.variables,
        context.nodeId,
      );
      ...
      console.log(
        `[CC] Sent to ${ccReceivers.length} CC receiver(s) for instance ${context.instanceId}`,
      );
    } catch (error) {
  ...
  private async getCcReceivers(
    props: CcNodeProperties,
