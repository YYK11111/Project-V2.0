...
export class NotificationNodeHandler implements INodeHandler {
  ...
  async execute(context: NodeExecutionContext): Promise<NodeResult> {
    const props = context.variables
      ._nodeProperties as NotificationNodeProperties;
    if (!props) {
      console.warn(
        `[Notification] No properties found for node ${context.nodeId}`,
      );
      return {
        success: true,
        nextNodeIds: [],
        outputData: { status: "skipped" },
      };
    }
    ...
    const title = this.renderTemplate(
      props.notificationTemplate || "流程通知",
      context.variables,
    );
    ...
    if (receivers.length === 0) {
      ...
      return {
        success: true,
        nextNodeIds: [],
        outputData: { status: "no_receivers" },
      };
    }
    ...
    try {
      ...
      const businessTitle =
        businessData.name || businessData.title || context.instanceId;
...
节点：${context.nodeId}`.trim();
      ...
      console.log(
        `[Notification] Sent to ${receivers.length} receiver(s) for instance ${context.instanceId}`,
      );
    } catch (error) {
      console.error(
        `[Notification] Failed to send notification: ${error.message}`,
      );
    }
  ...
  private async getNotificationReceivers(
    props: NotificationNodeProperties,
    ...
    return this.parseReceivers(
      props.notificationReceiverExpr || props.notificationReceivers,
      variables,
    );
  }
