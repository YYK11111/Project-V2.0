...
export class WorkflowService {
  ...
  private normalizeNodeProperties(node: NodeConfig): NodeConfig["properties"] {
    ...
    if (node.type === NodeType.APPROVAL) {
      ...
      return {
        ...
        fieldPath: "",
        businessType:
          restProperties.businessType || legacyConfig.businessType || "",
  ...
  private normalizeDefinition<
    T extends WorkflowDefinition | Partial<WorkflowDefinition>,
  >(definition: T): T {
  ...
  async getDefinitionByScene(
    ...
    if (!definitions.length) {
      throw new BadRequestException(
        `未找到已发布流程：${businessType} / ${businessScene}`,
      );
    }
  ...
  async copyDefinition(id: string): Promise<WorkflowDefinition> {
    ...
    const savedDefinition = await this.definitionRepo.save(
      normalizedCopy as WorkflowDefinition,
    );
    return this.normalizeDefinition(savedDefinition as WorkflowDefinition);
  }
  ...
  private async startWorkflowUnlocked(
    ...
    if (definition.businessType) {
      try {
        ...
        console.log(
          `[Workflow] Loaded business data for ${definition.businessType}: ${dto.businessKey}`,
        );
      } catch (error) {
        console.error(
          `[Workflow] Failed to load business data: ${error.message}`,
        );
        // 不阻止流程继续，但记录错误
      }
    }
  ...
  private async findNextNodes(
    ...
    // 否则根据连接线配置查找
    const outgoingFlows = definition.flows.filter(
      (f) => f.sourceNodeId === currentNode.id,
    );
    ...
    if (currentNode.type === NodeType.CONDITION) {
      ...
      if (conditions.length) {
        for (const condition of conditions) {
          ...
          if (
            !(await this.evaluateCondition(condition, instance.variables || {}))
          )
            continue;
          const flow = outgoingFlows.find(
            (f) => f.flowType === "condition" && f.conditionId === condition.id,
          );
        ...
        throw new BadRequestException(
          `条件节点「${currentNode.name}」未命中任何分支，且未配置默认分支`,
        );
      }
    }
    ...
    if (currentNode.type === NodeType.END && outgoingFlows.length > 0) {
      throw new BadRequestException(
        `结束节点「${currentNode.name}」不能存在流出连接线`,
      );
    }
    ...
    if (outgoingFlows.length > 1) {
      throw new BadRequestException(
        `节点「${currentNode.name}」存在多条流出连接线，仅条件节点允许多分支`,
      );
    }
  ...
  private async checkDeptContains(
    userDeptId: string,
    targetDeptId: string,
    variables: Record<string, any>,
  ): Promise<boolean> {
  ...
  private async checkUserDeptFromService(
    userId: string,
    deptId: string,
    includeChildren: boolean,
  ): Promise<boolean> {
  ...
  private async createApprovalTask(
    ...
    try {
      ...
      console.log(
        `[Workflow] Resolved ${assigneeIds.length} assignees for node ${node.id}`,
      );
    } catch (error) {
    ...
    if (assigneeIds.length === 0) {
      const emptyAction =
        props.assigneeEmptyAction || AssigneeEmptyAction.ERROR;
      ...
      switch (emptyAction) {
        ...
        case AssigneeEmptyAction.SKIP:
          console.warn(
            `[Workflow] No assignee found for node ${node.id}, skipping...`,
          );
          return;
        case AssigneeEmptyAction.ASSIGN_TO:
          ...
          console.log(
            `[Workflow] Using fallback assignee for node ${node.id}: ${assigneeIds.join(",")}`,
          );
          break;
      }
    }
  ...
  private async sendApprovalNotification(
    ...
    try {
      ...
      console.log(
        `[Workflow] Sent notification for task ${task.id} to user ${task.assigneeId}`,
      );
    } catch (error) {
  ...
  private async sendReturnToStarterMessage(
    instance: WorkflowInstance,
    task: WorkflowTask,
    comment?: string,
  ): Promise<void> {
...
请根据意见修改业务内容后继续处理。`,
      ...
      console.error(
        `[Workflow] Failed to send return-to-starter message: ${error.message}`,
      );
    }
  }
  ...
  private async completeTaskUnlocked(
    ...
    task.status =
      dto.action === "approve" ? TaskStatus.COMPLETED : TaskStatus.CANCELLED;
    task.action =
  ...
  async closeReturnedInstance(
    ...
    await this.workflowIntegrationService.handleCloseReturnedInstance(
      instance.id,
      {
        businessKey: instance.businessKey,
        ...(instance.variables || {}),
  ...
  async resubmitReturnedInstance(
    instanceId: string,
    userId: string,
    dto?: ResubmitReturnedWorkflowDto,
  ): Promise<WorkflowInstance> {
    const instance = await this.instanceRepo.findOne({
      where: { id: instanceId },
    });
    ...
    await this.instanceRepo.save(instance);
    await this.messagesService.deactivateWorkflowInstanceCcMessages(
      instance.id,
      userId,
    );

    await this.workflowIntegrationService.handleResubmitReturnedInstance(
      instance.id,
      {
        businessKey: instance.businessKey,
        ...(instance.variables || {}),
    ...
    const startNodeIndex = definition.nodes.findIndex(
  ...
  async getPendingTasks(userId: string): Promise<any[]> {
    ...
    return Promise.all(
      tasks.map((task) => this.attachBusinessSummaryToTask(task)),
    );
  }

  async getHandledHistory(
    userId: string,
    query: {
      pageNum?: number;
      pageSize?: number;
      ...
      await Promise.all(
        historyList
          ...
          .map(async (item) => {
            ...
            const summary = await this.getBusinessSummary(
              instance.businessKey,
              instance.variables,
            );
            const title = this.buildHandledHistoryTitle(
              summary.businessType,
              summary.businessTitle,
  ...
  private async attachBusinessSummaryToInstance(
    instance: WorkflowInstance,
  ): Promise<any> {
    const summary = await this.getBusinessSummary(
      instance.businessKey,
  ...
  private async getBusinessSummary(businessKey: string, variables?: any) {
    ...
    if (!businessKey)
      return { businessType: "", businessTitle: "", businessCode: "" };
  ...
  private getHistoryActionText(action: string, comment?: string) {
    const actionMap: Record<string, string> = {
      ...
      "5": "加签",
      "6": "终止",
      execute: String(comment || "").includes("发起人重新提交审批")
        ? "发起人重新提交"
        : "执行",
    };
  ...
  private buildHandledHistoryTitle(
    businessType: string,
  ...
  private async buildWorkflowMessageSummary(
    instance: WorkflowInstance,
    options: { nodeName?: string; actionLabel: "待办" | "待阅" },
  ) {
    const summary = await this.getBusinessSummary(
      instance.businessKey,
      instance.variables,
...
请及时处理。`,
  ...
  private async completeInstance(instance: WorkflowInstance): Promise<void> {
    ...
    instance.duration = String(endTime - startTime);
    await this.instanceRepo.save(instance);
    await this.workflowIntegrationService.handleWorkflowCallback(
      instance.id,
      "completed",
      {
        businessKey: instance.businessKey,
   ...
   * 获取业务对象配置列表
   */
  async listBusinessConfigs(
    businessType?: string,
  ): Promise<WorkflowBusinessConfig[]> {
    const where: any = {};
    if (businessType) {
  ...
  async createBusinessConfig(dto: any): Promise<WorkflowBusinessConfig> {
    ...
    return (await this.configRepo.save(
      config,
    )) as any as WorkflowBusinessConfig;
  }
