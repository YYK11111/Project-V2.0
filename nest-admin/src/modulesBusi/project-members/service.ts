...
> {
  ...
  async list(query: QueryListDto): Promise<ResponseListDto<ProjectMember>> {
    ...
    if (keyword) {
      qb.andWhere(
        "(project.name LIKE :keyword OR project.code LIKE :keyword OR user.name LIKE :keyword OR user.nickname LIKE :keyword)",
        {
          keyword: `%${keyword}%`,
        },
      );
    ...
    if (issueType === "missingManager" || issueType === "missingCore") {
      const projectRows = (await this.repository
        ...
        .addSelect(
          "SUM(CASE WHEN memberIssue.role = :managerRole THEN 1 ELSE 0 END)",
          "managerCount",
        )
        .addSelect(
          "SUM(CASE WHEN memberIssue.isCore = :coreValue THEN 1 ELSE 0 END)",
          "coreCount",
        )
        ...
        .orderBy("memberIssue.projectId", "ASC")
        .getRawMany()) as Array<{
        projectId: string;
        managerCount: string;
        coreCount: string;
      ...
      qb.andWhere("member.projectId IN (:...targetProjectIds)", {
        targetProjectIds,
      });
    }
  ...
  async getStats() {
    ...
    const projectRows = (await this.repository
      ...
      .addSelect(
        "SUM(CASE WHEN member.role = :managerRole THEN 1 ELSE 0 END)",
        "managerCount",
      )
      .addSelect(
        "SUM(CASE WHEN member.isCore = :coreValue THEN 1 ELSE 0 END)",
        "coreCount",
      )
      .where("member.is_delete IS NULL")
    ...
    const missingManagerProjects = projectRows.filter(
      (item) => Number(item.managerCount || 0) === 0,
    ).length;
    const missingCoreProjects = projectRows.filter(
      (item) => Number(item.coreCount || 0) === 0,
    ).length;
  ...
  async getProjectOverview(query: QueryListDto) {
    ...
    if (keyword) {
      qb.andWhere(
        "(project.name LIKE :keyword OR project.code LIKE :keyword)",
        { keyword: `%${keyword}%` },
      );
    }
    ...
    if (issueType === "missingManager") {
      qb.having(
        "SUM(CASE WHEN member.role = :managerRole THEN 1 ELSE 0 END) = 0",
        { managerRole: ProjectMemberRole.manager },
      );
    }
    if (issueType === "missingCore") {
      qb.having(
        "SUM(CASE WHEN member.isCore = :coreValue THEN 1 ELSE 0 END) = 0",
        { coreValue: "1" },
      );
    }
