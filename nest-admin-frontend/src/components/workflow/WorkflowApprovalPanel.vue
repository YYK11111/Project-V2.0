...
const loadWorkflowContext = async () => {
  ...
  if (instanceInfo.value?.definitionId) {
    ...
    rejectableNodes.value = (definition.nodes || []).filter((node: any) => executedNodeIds.has(node.id) && node.type !== 'end')
  }
}
...
const rejectableNodeOptions = computed(() => {
  return rejectableNodes.value.map((node: any, index: number) => ({
    value: node.id,
    label: `${index + 1}. ${node.name}（${nodeTypeLabelMap[node.type] || node.type}）`,
  }))
})

const submitDecision = async (action: 'approve' | 'reject') => {
  if (!props.taskId) return ElMessage.warning('缺少审批任务ID')
  loading.value = true
  try {
...
<template>
  <el-card class="workflow-approval-panel" shadow="hover">
    ...
    <el-form label-width="90px">
      ...
      <el-form-item>
        <el-button type="primary" :loading="loading" @click="submitDecision('approve')">同意</el-button>
        <el-button type="danger" :loading="loading" @click="rejectVisible = true">拒绝</el-button>
        <el-button type="warning" @click="transferVisible = true">转交</el-button>
        <el-button type="info" @click="addSignVisible = true">加签</el-button>
