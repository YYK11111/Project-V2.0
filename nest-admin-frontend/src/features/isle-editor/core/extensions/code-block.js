    ...
    return {
      ...
      command: ({ editor, range }) => {
        ...
        if (range) {
          ...
          return chain.insertContent({ type: 'codeBlock' }).run()
        }
  ...
  addKeyboardShortcuts() {
    return {
      ...
      Enter: ({ editor }) => {
        ...
        const lineStart = textContent.lastIndexOf('\n', Math.max(cursorOffset - 1, 0)) + 1
        const nextBreakIndex = textContent.indexOf('\n', cursorOffset)
        const lineEnd = nextBreakIndex === -1 ? textContent.length : nextBreakIndex
        const currentLineText = textContent.slice(lineStart, lineEnd)
