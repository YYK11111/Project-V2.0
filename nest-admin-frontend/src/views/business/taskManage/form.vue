...
const canSubmitCurrentApproval = computed(() => String(form.value.status || '') === '1' && !['1', '2'].includes(String(form.value.approvalStatus || '0')))
const canCloseReturnedInstance = computed(() => String(form.value.workflowInstanceId || '') && form.value.approvalStatus === '3' && String(form.value.currentNodeName || '').includes('退回发起人'))
