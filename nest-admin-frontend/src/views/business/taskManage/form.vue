...
const canEditCurrentTask = computed(() => !hasTaskId.value || (form.value?.permissionContext?.canEdit ?? form.value?.canEdit) !== false)
const canExecuteCurrentTask = computed(() => hasTaskId.value && (form.value?.permissionContext?.canExecute ?? form.value?.canExecute) !== false)
const canStartTask = computed(() => hasTaskId.value && canExecuteCurrentTask.value && String(form.value.status || '') === '1')
const canPauseTask = computed(() => hasTaskId.value && (form.value?.permissionContext?.canManage ?? form.value?.canManage) !== false && String(form.value.status || '') === '2')
const canResumeTask = computed(() => hasTaskId.value && canExecuteCurrentTask.value && String(form.value.status || '') === '5')
const canDelayTask = computed(() => hasTaskId.value && (form.value?.permissionContext?.canManage ?? form.value?.canManage) !== false && ['1', '2', '5', '6'].includes(String(form.value.status || '')))
