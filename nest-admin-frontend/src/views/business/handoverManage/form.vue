...
function handleSubmitApproval() {
  ...
  $sdk.msgWarning(`提交审批前，请先确认已补齐：${handoverApprovalRequirements.join('、')}`)
...
<template>
  <FormPageShell class="business-form-page handover-form-shell">
    <template #footerMeta>
      <span>{{ isReadonly ? '查看模式' : isEdit ? '编辑模式' : '新建模式' }}</span>
      <span v-if="fromWorkflow && workflowTaskId">当前来源于流程审批</span>
    </template>

    <el-page-header class="business-form-header" @back="$router.back()" :title="isReadonly ? '运维交接单详情' : isEdit ? '编辑运维交接单' : '新增运维交接单'">
      <template #extra>
    ...
    <el-form ref="formRef" :model="form" :rules="rules" label-width="120px" class="business-form">
      <div class="business-form-sections">
        <section class="business-form-section">
          <div class="business-form-section__header">
            <div>
              ...
              <div class="business-form-section__desc">维护交接标题、所属项目、接维对象、交接日期和状态，先明确运维承接范围。</div>
            </div>
          </div>
        ...
        <section class="business-form-section">
          <div class="business-form-section__header">
            <div>
              <div class="business-form-section__title">服务交接内容</div>
              <div class="business-form-section__desc">沉淀服务窗口、联系人渠道、已知问题和知识准备情况，降低交接后的支持风险。</div>
            </div>
          </div>
