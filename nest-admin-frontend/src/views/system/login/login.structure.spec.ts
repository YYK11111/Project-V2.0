...
describe('login branding structure', () => {
  it('登录页不再渲染原项目说明文案', () => {
    ...
    expect(source).toContain('feishuLogo')
    expect(source).toContain('@/assets/image/feishu.png')
    expect(source).toContain('alt="飞书"')
    expect(source).not.toContain('<span>飞书登录</span>')
    expect(source).not.toContain("icon=\"dingtalk\"")
    expect(source).not.toContain('<span>钉钉</span>')
    expect(source).toContain('external-login-button')
