...
describe('system configs structure', () => {
  ...
  it('系统配置支持维护外部通知配置', () => {
    ...
    expect(source).toContain("enabledScenes: ['workflow.approval.todo']")
    expect(source).toContain('const externalNotifyScenes = [')
    expect(source).toContain('消息场景')
    expect(source).toContain('流程审批待办')
    expect(source).toContain('模板状态')
